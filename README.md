...
---
> Оригинальную версию в формате pdf можно найти в файле this-repo/tex/main.pdf

## Содержание
- [Аннотация](#аннотация)
  - [Цели работы](#цели-работы)
  - [Теоретическая справка](#теоретическая-справка)
    - [Последовательная модель выполнения (SEQ)](#последовательная-модель-выполнения-seq)
    - [Конвейерная модель выполнения (PIPE)](#конвейерная-модель-выполнения-pipe)
    - [Оптимизации](#оптимизации)
- [Оборудование](#оборудование)
- [Ход работы](#ход-работы)
  - [Зависимости](#зависимости)
  - [Начало работы. Поверхностная оценка производительности](#начало-работы-поверхностная-оценка-производительности)
    - [Нулевая версия](#нулевая-версия)
    - [Базовые оптимизации](#базовые-оптимизации)
    - [Векторные инструкции (AVX2)](#векторные-инструкции-avx2)
    - [Развёртывание (loop unrolling)](#развёртывание-loop-unrolling)
    - [Кроссплатформенность](#кроссплатформенность)
  - [Точная оценка производительности](#точная-оценка-производительности)
    - [Проверка стационарности](#проверка-стационарности)
    - [Измерения](#измерения)
      - [Baseline](#baseline)
      - [Ручные оптимизации](#ручные-оптимизации)
- [Вывод](#вывод)
...
На рисунке ниже представлена последовательная модель обработки инструкций:

![Блок-схема последовательной обработки инструкций (SEQ)[2]](readme_assets/seq.png)
...
Современные процессоры используют конвейерную обработку, как показано на рисунке ниже.

![Блок-схема конвейерной обработки инструкций (PIPE)[2]](readme_assets/pipe.png)

Из общих изменений в схеме добавились конвейерные регистры между каждой соседней парой этапов обработки инструкций. Это сделано для возможности выполнения разных этапов разных инструкций параллельно, как показано на рисунке ниже.

![Визуализация конвейерной обработки инструкций[2]](readme_assets/pipeline.png)
...
Но схема выше будет работать корректно только если во всех инструкциях, которые выполняются параллельно не будет *зависимости по данным*, что может часто не соблюдаться. Приведём методы разрешения таких зависимостей.

- **NOP (No Operation):** самое лёгкое решение. На этапе генерации ассемблера проставить пустые инструкции в количестве необходимом, для такого чтобы устранить параллельное выполнение инструкций зависимых по данным.

![Останова, реализованная через NOP для решения проблемы риска по данным[2]](readme_assets/nop.png)

- **"Пузырёк" (bubble, Привет дед!):** Аппаратная приостановка конкретных этапов обработки инструкций, зависящих по данным.

![Останова, реализованная через "пузырёк" для решения проблемы риска по данным[2]](readme_assets/babble.png)

- **Продвижение (forwarding):** Мы добавляем в нашу конструкцию процессора возможность передачи значений (регистров, либо памяти) в предыдущие этапы обработки инструкций, чтобы ожидающая инструкция могла использовать уже вычисленное значение до окончания обработки ожидаемой инструкции.

![Визуализация продвижения назад при зависимости по данных[3]](readme_assets/forwarding.png)
...
Распределением инструкций по функциональным блокам занимается *блок управления инструкциями (БУИ)* (Instruction Control Unit). В частности на всех современных процессорах есть блок для обработки ветвлений, который пытается предсказать условные переходы (аналогично предсказателю PC в рамках одного конвейера, но на уровень выше). Такая организация обработки инструкций называется *спекулятивным выполнением (speculative execution)*.

![Блок-схема суперскалярного процессора[2]](readme_assets/supscalar.png)
...
- [18+ бутылка пива](https://www.tver.ru/info/prokuratura/proletarskiy-rayon/razyasneniya/39923/)
- [чипсы соседа](https://github.com/71frukt)
- третье издание БХ [2]
- [слинки (всем с СДВГ советую)](https://www.mirpruzhin.ru/)
- ноутбук (опционально)

![Характеристики ноутбука, на котором выполняется работа](readme_assets/nout.png)
...
- Вывод текущего FPS, чтобы примерно оценить скорость расчёта кадров без точных времязатраных измерений, реализованный через функции SDL2.

![Визуализация множества Мандельброта без оптимизаций](readme_assets/mand_O0.png)
...
![Визуализация множества Мандельброта c наивными оптимизациями](readme_assets/mand_O3.png)
...
Будем использовать тип `__m256`, который сможет хранить 8 значений типа `float`. Соответственно итоговый прирост к скорости должен быть примерно в 8 раз по сравнению с прошлой версией без учёта накладных расходов.

![Визуализация множества Мандельброта на интринсиках](readme_assets/mand_avx2.png)
...
![Визуализация множества Мандельброта на интринсиках c развёртыванием](readme_assets/mand_avx2_unroll.png)
...
![Визуализация множества Мандельброта на массивах c развёртыванием](readme_assets/mand_arr2.png)
...
![График зависимости времени выполнения от итерации](readme_assets/stat_check.png)
...
Имеет смысл оценивать прирост производительности оптимизаций, которые требуют переработки кода, так как оптимизирующие флаги компиляции и так используются в release-версии программы, поэтому выберем baseline, то есть версию, относительно которое будем смотреть прирост оптимизаций.

![Сравнение производительности версий программы с базовыми оптимизациями](readme_assets/histogram0.png)

| Номер | Компилятор | Флаг -O | Оптимизации | Производительность | Время (такт/кадр·10⁶) |
|-------|------------|---------|-------------|--------------------|-----------------------|
...
![Сравнение производительности версий программы с ручными оптимизациями](readme_assets/histogram1.png)
...
1. [Сюда звёздочки ставить](https://github.com/kzueirf12345/mandelbrat2)
2. [Компьютерные системы. Архитектура и программирование [2022] Брайант Р. Э., О'Халларон Д. Р.](https://vk.com/wall-51126445_124413)
3. [(Визуализация продвижения)](https://www.researchgate.net/figure/Data-Forwarding-1-With-a-forwarding-unit-these-stall-cycles-can-be-alleviated-See_fig2_308963446)
4. [Зеркало сайта с интрисинками на intel](https://www.laruence.com/sse/#techs=MMX,SSE,SSE2,SSE3,SSSE3,SSE4_1,SSE4_2,AVX,AVX2&expand=4974,4923,3865,3869,3870,736&text=mul)